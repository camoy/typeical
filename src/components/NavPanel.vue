    ...
    <v-alert type="info" class="infoBar" id="infoBar">
      ...
      <v-tooltip bottom>
        ...
        <span>
          ...
          <br />
          Flow's width is proportional to the frequency of the type signature in
          the data set.
        </span>
      </v-tooltip>
    </v-alert>
    ...
    <div id="treemapsDiv">
      ...
      <div class="treemap-div">
        ...
        <svg class="treemap-svg">
          ...
          <g v-if="pkgRoot.children">
            <g
              v-for="(leaf, k) in pkgRoot.leaves()"
              :key="'pkg-treemap-g' + k"
      ...
      <div class="treemap-div">
        ...
        <svg class="treemap-svg" v-if="funRoot.children">
          <!-- Data -->
          <g
            v-for="(leaf, k) in funRoot.leaves()"
            :key="'fun-treemap-g' + k"
            :transform="`translate(${leaf.x0}, ${leaf.y0})`"
          >
            <title>{{ leafName(leaf) }} ({{ leafValue(leaf) }})</title>
...
function colorPkgText(leaf) {
  return isSelectedPkg.call(this, leaf)
    ? SELECTED_TEXT_COLOR
    : DEFAULT_TEXT_COLOR;
}
...
function selectFun(leaf) {
  ...
  this.selectedFuns = this.selectedFuns.includes(fun)
    ? base.filter(x => x !== fun)
    : base.concat([fun]);
}
...
function colorFunText(leaf) {
  return isSelectedFun.call(this, leaf)
    ? SELECTED_TEXT_COLOR
    : DEFAULT_TEXT_COLOR;
}
...
function makeTree(xs, name, page) {
  let children = xs
    ? xs.map(x => {
        return { name: x[name], value: Math.log(x.count) + 1 };
      })
    : [];
...
export default {
  ...
  watch: {
    ...
    selectedPkg() {
      if (this.selectedFuns.length === 0) this.$store.dispatch("queryTypes");
  ...
  computed: {
    ...
    selectedPkg: {
      get() {
        return this.$store.state.selectedPkg;
      },
      set(value) {
        this.$store.dispatch("setSelectedPkg", value);
      }
    },
    ...
    selectedFuns: {
      get() {
        return this.$store.state.selectedFuns;
      },
      set(value) {
        this.$store.dispatch("setSelectedFuns", value);
      }
    },
